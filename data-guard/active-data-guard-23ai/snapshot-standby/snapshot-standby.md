...
1. From a terminal (**the host is irrelevant**), connect with `dgmgrl` and convert the standby database to a snapshot standby. **Don't forget to replace `ADGHOL1_UNIQUE_NAME` with the actual `db_unique_name` of the standby database.**
...
1. Connect to the standby database. **Replace `ADCHOL1_DGCI` with the correct connection string.**
    ```
    <copy>
