...
1. **On the primary host** `adghol0`, get the `DB_UNIQUE_NAME` of the primary database. On OCI BaseDB, the environment variable `$ORACLE_UNQNAME` is set to the correct value:
...
In this workshop, we use easy connect (EZCONNECT) as the naming method for the Data Guard configuration. Using EZCONNECT has the advantage that it does not require modifications to the `tnsnames.ora` or other naming methods like LDAP.
...
1. **On the primary host** (adghol0), execute the following command to build the `DGConnectIdentifier` of the primary database:
   ...
   **We will refer to it as ADGHOL0_DGCI** ( **DGC**onnect**I**dentifier).

2. Repeat the command, but appending **_DGMGRL** to the **DB_UNIQUE_NAME** this time, to build the `StaticConnectIdentifier` of the primary database:
   ```
   <copy>
   ...
   ```
   Note it down for future reference.
   **We will refer to it as ADGHOL0_SCI** ( **S**tatic**C**onnect**I**dentifier).

   ![Commands to build the connect identifiers](images/connect-identifiers.png)

3. Repeat the two above commands on the second host to get the connect identifiers for the standby database:
